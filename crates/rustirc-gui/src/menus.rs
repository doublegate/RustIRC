...
impl MenuBar {
    ...
    fn render_file_menu(&self, app_state: &AppState) -> Element<MenuMessage> {
        ...
        let connected = app_state.servers.values().any(|s| s.connection_state == rustirc_core::ConnectionState::Registered);
        ...
        column![
            ...
            button(text(disconnect_text))
                .on_press(if connected { MenuMessage::FileDisconnect } else { MenuMessage::FileConnect }),
    ...
    fn render_edit_menu(&self, app_state: &AppState) -> Element<MenuMessage> {
        ...
        let has_selection = app_state.current_tab()
            .map(|tab| !tab.messages.is_empty())
            .unwrap_or(false);
        
        column![
            button(text(if has_selection { "Copy" } else { "Copy (no selection)" }))
    ...
    fn render_view_menu(&self, app_state: &AppState) -> Element<MenuMessage> {
        ...
        column![
            button(text(if show_sidebar { "☑ Server Tree" } else { "☐ Server Tree" }))
                .on_press(MenuMessage::ViewToggleServerTree),
            button(text(if show_userlist { "☑ User List" } else { "☐ User List" }))
    ...
    fn render_server_menu(&self, app_state: &AppState) -> Element<MenuMessage> {
        ...
        let current_server = app_state.current_tab()
        ...
        column![
            ...
            button(text(format!("Disconnect {}", if connected { &server_name } else { "" })))
                .on_press(MenuMessage::ServerDisconnect),
            button(text(format!("Reconnect{}", if connected { "" } else { " (Not Connected)" })))
                .on_press(MenuMessage::ServerReconnect),
            button(text("Properties...")).on_press(MenuMessage::ServerProperties),
            button(text(format!("Add {} to Favorites", server_name)))
    ...
    fn render_channel_menu(&self, app_state: &AppState) -> Element<MenuMessage> {
        ...
        let current_channel = app_state.current_tab()
            .and_then(|tab| {
        ...
        let user_count = app_state.current_tab()
            .map(|tab| tab.users.len())
            .unwrap_or(0);
        
        column![
            button(text("Join Channel...")).on_press(MenuMessage::ChannelJoin),
            button(text(format!("Part {}", if in_channel { &channel_name } else { "Channel" })))
                .on_press(MenuMessage::ChannelPart),
            button(text(format!("{} Topic...", if in_channel { &channel_name } else { "Channel" })))
                .on_press(MenuMessage::ChannelTopic),
            button(text(format!("Channel Modes... ({})", if in_channel { "+nt" } else { "none" })))
                .on_press(MenuMessage::ChannelModes),
            button(text(format!("Ban List... ({} users)", user_count)))
    ...
    fn render_tools_menu(&self, app_state: &AppState) -> Element<MenuMessage> {
        ...
        let log_count = app_state.tabs.values()
        ...
        column![
            ...
            button(text(format!("Log Viewer... ({} messages)", log_count)))
                .on_press(MenuMessage::ToolsLogViewer),
            button(text(format!("Network List... ({} servers)", app_state.servers.len())))
    ...
    fn render_help_menu(&self, _app_state: &AppState) -> Element<MenuMessage> {
        ...
        column![
            ...
            button(text(format!("About RustIRC v{}...", VERSION)))
                .on_press(MenuMessage::HelpAbout),
        ]
        .spacing(2)
        .into()
