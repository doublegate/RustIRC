...
use iced::{Color, Element, widget::{text, button}};
use regex::Regex;
        ...
        match ch {
            ...
            codes::COLOR => {
                ...
                if let Some(fg_color) = fg {
                    current_span.foreground = Some(IRC_COLORS.get(fg_color).copied().unwrap_or(Color::WHITE));
                }
                if let Some(bg_color) = bg {
                    current_span.background = Some(IRC_COLORS.get(bg_color).copied().unwrap_or(Color::BLACK));
                }
            }
            codes::RESET => {
                if !current_span.text.is_empty() {
...
fn parse_color_codes(chars: &mut std::iter::Peekable<std::str::Chars>) -> (Option<usize>, Option<usize>) {
...
pub fn strip_formatting(text: &str) -> String {
    ...
    while let Some(ch) = chars.next() {
        match ch {
            ...
            codes::BOLD | codes::ITALIC | codes::UNDERLINE | 
            codes::STRIKETHROUGH | codes::MONOSPACE | 
            codes::REVERSE | codes::RESET => {
