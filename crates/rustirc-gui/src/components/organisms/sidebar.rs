...
impl ModernSidebar {
        ...
        match message {
            SidebarMessage::ServerToggled(server_id) => {
                if let Some(server) = self.servers.iter_mut().find(|s| s.id == server_id) {
                    ...
                    let current_height = self.animations
                        .get(&server_id)
                        .map(|anim| anim.current_height)
                        .unwrap_or(if server.is_expanded { 0.0 } else { target_height });
    ...
    fn expanded_view(&self) -> Element<'_, SidebarMessage, Theme, Renderer> {
        ...
        for server in &self.servers {
            ...
            if server.is_expanded || self.animations.contains_key(&server.id) {
                let channel_height = self.animations
                    .get(&server.id)
                    .map(|anim| anim.current_height)
                    .unwrap_or_else(|| {
    ...
    fn create_server_item(&self, server: &ServerInfo) -> Element<'_, SidebarMessage, Theme, Renderer> {
        ...
        let content = row![
            ...
            // Status indicator
            container(text("●").size(12).color(status_color))
                .padding(4),
    ...
    fn create_channel_list(&self, server: &ServerInfo, height: f32) -> Element<'_, SidebarMessage, Theme, Renderer> {
    ...
    fn create_channel_item(&self, server: &ServerInfo, channel: &ChannelInfo) -> Element<'_, SidebarMessage, Theme, Renderer> {
        let is_selected = self.selected_server.as_ref() == Some(&server.id) 
        ...
        .on_press(SidebarMessage::ChannelSelected(server.id.clone(), channel.id.clone()))
        .into()
    }

    fn create_compact_server_item(&self, server: &ServerInfo) -> Element<'_, SidebarMessage, Theme, Renderer> {
        let is_selected = self.selected_server.as_ref() == Some(&server.id);
    ...
    fn create_unread_badge(&self, count: u32) -> Element<'_, SidebarMessage, Theme, Renderer> {
        let count_text = if count > 99 { "99+".to_string() } else { count.to_string() };
    ...
    fn create_mention_badge(&self, count: u32) -> Element<'_, SidebarMessage, Theme, Renderer> {
        let count_text = if count > 9 { "9+".to_string() } else { count.to_string() };
