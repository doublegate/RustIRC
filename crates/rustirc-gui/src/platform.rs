...
impl NotificationManager {
    ...
    pub fn show_message_notification(&self, sender: &str, channel: &str, message: &str) -> Result<()> {
        let title = if channel.starts_with('#') {
            format!("{} in {}", sender, channel)
        } else {
    ...
    pub fn show_highlight_notification(&self, sender: &str, channel: &str, message: &str) -> Result<()> {
        let title = format!("🔔 Highlighted in {} by {}", channel, sender);
...
impl DesktopIntegration {
    ...
    fn register_windows_protocol() -> Result<()> {
        ...
        let commands = vec![
            ...
            format!(r#"reg add "HKEY_CLASSES_ROOT\irc\shell\open\command" /ve /d "\"{}\" \"%1\"" /f"#, exe_path.display()),
        ];
...
"#;
        ...
        let desktop_file = format!("{}/.local/share/applications/rustirc.desktop", home);
...
"#;
        ...
        let home = std::env::var("HOME")?;
        let desktop_file = format!("{}/Desktop/RustIRC.desktop", home);
        std::fs::write(desktop_file, desktop_content)?;
...
"#;
        ...
        let autostart_file = format!("{}/rustirc.desktop", autostart_dir);
    ...
    fn remove_linux_autostart() -> Result<()> {
        let home = std::env::var("HOME")?;
        let autostart_file = format!("{}/.config/autostart/rustirc.desktop", home);
