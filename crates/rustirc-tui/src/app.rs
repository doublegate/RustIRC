...
impl TuiApp {
        ...
        match parts[0] {
            ...
            _ => {
                if command.starts_with('/') {
                    ...
                    error!("Unrecognized command: {}. Type /help for available commands", parts[0]);
                } else {
    ...
    pub fn get_backend_info(&self) -> String {
        if let Some(ref terminal) = self.terminal {
            ...
            info!("Backend initialized with size: {}x{}", size.width, size.height);
    ...
    pub async fn get_irc_status(&self) -> String {
        ...
        let current_server = current_state.current_server.unwrap_or_else(|| "none".to_string());
        
        format!("IRC Client: Connected | Servers: {} | Current: {}", server_count, current_server)
    }
    ...
    pub async fn process_pending_commands(&self) -> usize {
        ...
        if let Err(_) = self.command_sender.send("PING".to_string()) {
        ...
        let status_report = format!("Processed {} commands", processed);
